...
public class MainActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle b) {
        ...
        s.setDatabaseEnabled(true); s.setLoadsImagesAutomatically(true); s.setJavaScriptCanOpenWindowsAutomatically(false); s.setCacheMode(WebSettings.LOAD_NO_CACHE);
    ...
    private void deliverPendingAuthUrl() { if (webView == null || pendingAuthUrl == null) return; String u = JSONObject.quote(pendingAuthUrl); pendingAuthUrl = null; webView.post(() -> webView.evaluateJavascript("window.GamoraVetAuth12&&window.GamoraVetAuth12.handleDeepLink(" + u + ");", null)); }
    ...
    public static class GamoraVetBridge {
        ...
        GamoraVetBridge(MainActivity a){this.a=a;}
        ...
        private void performAuth(String requestId,String action,String payloadJson){
            int status=0; String body="";
            try {
                if(!supabaseConfigured()) throw new IllegalStateException("Configuração do servidor indisponível nesta compilação.");
                JSONObject p=new JSONObject(payloadJson==null?"{}":payloadJson); String endpoint; String method="POST"; JSONObject payload=new JSONObject();
                if("signup".equals(action)){endpoint="/auth/v1/signup";payload.put("email",p.getString("email"));payload.put("password",p.getString("password"));JSONObject data=new JSONObject();data.put("full_name",p.optString("full_name",""));data.put("role",p.optString("role","tutor"));payload.put("data",data);payload.put("gotrue_meta_security",new JSONObject().put("captcha_token",p.optString("captcha_token","")));}
                else if("login".equals(action)){endpoint="/auth/v1/token?grant_type=password";payload.put("email",p.getString("email"));payload.put("password",p.getString("password"));}
                else if("recover".equals(action)){endpoint="/auth/v1/recover?redirect_to="+URLEncoder.encode("gamoravet://auth/callback","UTF-8");payload.put("email",p.getString("email"));}
                else if("refresh".equals(action)){endpoint="/auth/v1/token?grant_type=refresh_token";payload.put("refresh_token",p.getString("refresh_token"));}
                else if("updatePassword".equals(action)){endpoint="/auth/v1/user";method="PUT";payload.put("password",p.getString("password"));}
                else throw new IllegalArgumentException("Ação de autenticação inválida.");
                URL url=new URL(BuildConfig.SUPABASE_URL+endpoint);HttpURLConnection c=(HttpURLConnection)url.openConnection();c.setRequestMethod(method);c.setConnectTimeout(15000);c.setReadTimeout(20000);c.setDoOutput(true);c.setRequestProperty("Content-Type","application/json");c.setRequestProperty("apikey",BuildConfig.SUPABASE_PUBLISHABLE_KEY);String access=p.optString("access_token","");if(!access.isEmpty())c.setRequestProperty("Authorization","Bearer "+access);byte[] bytes=payload.toString().getBytes(StandardCharsets.UTF_8);try(OutputStream os=c.getOutputStream()){os.write(bytes);}status=c.getResponseCode();InputStream is=status>=200&&status<300?c.getInputStream():c.getErrorStream();body=readAll(is);c.disconnect();
            } catch(Exception e){body=new JSONObjectSafe().error(e.getMessage());}
            callback(requestId,status,body);
        }
        ...
        private static class JSONObjectSafe { String error(String m){try{return new JSONObject().put("message",m==null?"Falha de comunicação.":m).toString();}catch(Exception e){return "{\"message\":\"Falha de comunicação.\"}";}} }
    }
